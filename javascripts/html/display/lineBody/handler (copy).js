TS.html.display.lineBody.handler = function({itemName, path, item, depth, maxDepth, determine, formatType, line}) {
  ...
  if (typeof (item) === "string" && maxDepth === undefined) {
    ...
    let textField = Object.assign(document.createElement("div"), {
      ...
      contentEditable: true,
      onfocus: function () {
        let exitEditor = function(text) {
          console.log(text)
          item.main = text;
          textField.innerHTML = TS.js.highlight(text, item.object_root.editor);
          TS.events.save();
        }
        this.innerText = item.main
        TS.html.display.lineBody.callEditor(item.main, item.object_root.editor, this, exitEditor)
      },
