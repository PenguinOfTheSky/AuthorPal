...
Object.assign(TS.js, {
  ...
  fileFormat: {
    markdownBlog: function (file) {
      ...
      let templates = {};
      let recent = ``; { // Todo: defined but never used?
            ...
            { //populate templates
              ...
              if (keys[i] === "#general" && keys[i] === "#advanced" && keys[i] === "master_root") {
                ...
                for (let x in file[keys[i]]) {
                  if (Object.prototype.hasOwnProperty.call(file[keys[i]], x)) {
                    if (x[0] === "*") {
                      str += file[keys[i]][x];
                    } else {
        ...
        for (let x in templates) {
          if (Object.prototype.hasOwnProperty.call(templates, x)) {
            body1 += templates[x];
          }
        }
      }
    ...
    <h2 id='tableOfContents'>Table of Contents</h2>`;
      let format = function (obj, depth, path, parent) {
        ...
        keys.forEach(ele => {
          if (ele === "master_root" || ele === "#advanced")
            return 0;
